...
test("Getting summary and Matched Transaction details from Stage Env", async ({ page }) => {
  ...
  const bankStatementfilefolder = path.resolve(__dirname, "../testData/Sea_Breeze_CSV/BankStatementFiles");
  const rfmsFileFolder = path.resolve(__dirname, "../testData/Sea_Breeze_CSV/RFMSFiles");
  const billingSystemFileFolder = path.resolve(__dirname, "../testData/Sea_Breeze_CSV/BillingSystemFiles");
  const testEnvData = path.resolve("./output/Sea_Breeze_CSV/TestEnvData.xlsx");
  const cashReconReport = path.resolve("./output/Sea_Breeze_CSV/Cash_Reconciliation_Report.xlsx");
  const testEnvDatareload = path.resolve("./output/Sea_Breeze_CSV/TestEnvData_Database.xlsx");
  const cashReconReportreload = path.resolve("./output/Sea_Breeze_CSV/Cash_Reconciliation_Report_Database.xlsx");
...
test("Comparing Stage Env Data with Prod Env Data", async ({ page }) => {
  ...
  const testEnvDatareload = path.resolve("./output/Sea_Breeze_CSV/TestEnvData_Database.xlsx");
  const cashReconReportreload = path.resolve("./output/Sea_Breeze_CSV/Cash_Reconciliation_Report_Database.xlsx");
  const mismatchFile = path.resolve("./output/Mismatches/Sea_Breeze_CSV_MismatchResult.xlsx");
  const mismatchFileDatabase = path.resolve("./output/Mismatches/Sea_Breeze_CSV_MismatchResult_Database.xlsx");
