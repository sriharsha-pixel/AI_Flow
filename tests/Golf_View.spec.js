...
test("Getting summary and Matched Transaction details from Stage Env", async ({ page }) => {
  ...
  const bankStatementfilefolder = path.resolve(__dirname, "../testData/Golf_View/BankStatementFiles");
  const rfmsFileFolder = path.resolve(__dirname, "../testData/Golf_View/RFMSFiles");
  const billingSystemFileFolder = path.resolve(__dirname, "../testData/Golf_View/BillingSystemFiles");
  const testEnvData = path.resolve("./output/Golf_View/TestEnvData.xlsx");
  const cashReconReport = path.resolve("./output/Golf_View/Cash_Reconciliation_Report.xlsx");
  const testEnvDatareload = path.resolve("./output/Golf_View/TestEnvData_Database.xlsx");
  const cashReconReportreload = path.resolve("./output/Golf_View/Cash_Reconciliation_Report_Database.xlsx");
...
test("Comparing Stage Env Data with Prod Env Data", async ({ page }) => {
  ...
  const testEnvDatareload = path.resolve("./output/Golf_View/TestEnvData_Database.xlsx");
  const cashReconReportreload = path.resolve("./output/Golf_View/Cash_Reconciliation_Report_Database.xlsx");
  const mismatchFile = path.resolve("./output/Mismatches/Golf_View_MismatchResult.xlsx");
  const mismatchFileDatabase = path.resolve("./output/Mismatches/Golf_View_MismatchResult_Database.xlsx");
